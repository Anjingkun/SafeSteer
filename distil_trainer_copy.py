...
class DynamicRefusalVectorCallback(TrainerCallback):
    """                                                                                                                                                                                                                                                                                                                                                                                                                          
      A HuggingFace TrainerCallback that periodically recomputes the "refusal direction"                                                                                                                                                                                                                                                                                                                                           
      used for activation-level steering during training. 
    """
    ...
    def __init__(self, trainer, harmful_train, harmless_train, harmful_val, harmless_val):
        ...
        if not safe_system_prompt:
            raise ValueError(
                ...
                "to be set (use --prompt_version V1/V2/V3 when use_refusal_vector=False)."
            )
...
class DistilTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        # SafeSteer✅ get the refusal vector usage flag from args, defaulting to True. This controls how we construct the teacher model
        self.use_refusal_vector = getattr(args, "use_refusal_vector", False)

        if self.use_refusal_vector and self.ref_model is not None :
            # SafeSteer✅ use the refusal vector to construct the teacher model.
            with torch.no_grad():
                # SafeSteer✅ compute the initial refusal direction
                harmful_train, harmless_train, harmful_val, harmless_val = self.load_and_sample_datasets_for_refusal_direction(args.seed)
                unwrapped_ref_model = self.accelerator.unwrap_model(self.ref_model)
                model_base = construct_model_base(unwrapped_ref_model, self.processing_class, args.model_name)
                harmful_train_filted, harmless_train_filted, harmful_val_filted, harmless_val_filted = filter_data(model_base, harmful_train, harmless_train, harmful_val, harmless_val, self.accelerator.is_main_process)
                candidate_directions = generate_and_save_candidate_directions(model_base, harmful_train_filted, harmless_train_filted)
                step_artifact_dir = os.path.join(args.output_dir, "refusal_direction", f"step_init")
                if self.accelerator.is_main_process:
                    print(f"SafeSteer✅ Initial refusal vector generated. Saving to {step_artifact_dir}...")
                    if not os.path.exists(os.path.join(step_artifact_dir, 'generate_directions')):
                        os.makedirs(os.path.join(step_artifact_dir, 'generate_directions'))
                    torch.save(candidate_directions, os.path.join(step_artifact_dir, 'generate_directions/mean_diffs.pt'))
                pos, layer, direction = select_and_save_direction(model_base, harmful_val_filted, harmless_val_filted, candidate_directions, step_artifact_dir, is_main_process=self.accelerator.is_main_process)
                torch.cuda.empty_cache()

            # SafeSteer✅ Initialize the refusal_state with the computed initial refusal vector
            self.refusal_state = {
            ...
            # SafeSteer✅ Register the hook to inject the refusal vector
            def teacher_act_add_hook(module, input_args):
                ...
                # SafeSteer✅ Ensure device and dtype match
                direction = self.refusal_state["direction"].to(
                    dtype=hidden_states.dtype, device=hidden_states.device
                )
                hidden_states += self.refusal_state["coeff"] * direction
                
                # SafeSteer✅ Keep all the original parameters！
                if isinstance(input_args, tuple):
                    return (hidden_states, *input_args[1:])
                else:
                    return hidden_states

            # SafeSteer✅ Register the hook exactly at the selected layer
            ...
            print(f"SafeSteer✅ Successfully attached the ActAdd Hook to the Teacher model (Layer {self.refusal_state['layer_idx']}). Injection coefficient: {self.refusal_state['coeff']}")
        elif (not self.use_refusal_vector) and self.args.voca_selection_mode == 2 and self.ref_model is not None:
            # SafeSteer✅ Prompt-based safe-token branch (no hook).
            safe_system_prompt = getattr(self.args, "safe_system_prompt", None)
            if not safe_system_prompt:
                raise ValueError("voca_selection_mode=2 with use_refusal_vector=False requires --prompt_version (V1/V2) so safe_system_prompt is set.")
            ...
            if self.accelerator.is_main_process:
                print(f"SafeSteer✅ Successfully computed initial safe_tokens via system-prompt steering (K={self.args.voca_selection_num}, horizon={self.args.safe_token_horizon})")
        # ===================================================================

        if args.sync_ref_model:
            self.add_callback(MemoryEfficientSyncRefModelCallback(ref_model=self.ref_model, accelerator=self.accelerator))
            
            # 2. 再加你的动态重算 Callback (它后执行，基于新权重算向量)
            # Only when use_refusal_vector AND update_refusal_vector are both True;
            # otherwise the init-time direction (and safe_tokens) is frozen.
            if (self.use_refusal_vector
                    and getattr(self.args, "update_refusal_vector", True)
                    and self.ref_model is not None):
                # Load and sample datasets

                dynamic_callback = DynamicRefusalVectorCallback(
                    trainer=self,
            ...
            elif self.use_refusal_vector and self.ref_model is not None:
                if self.accelerator.is_main_process:
                    print("⏸️ update_refusal_vector=False → 跳过动态重算 Callback; direction 冻结在 init 值。")
            
            # Decoupled safe-token refresh: independent of refusal-direction recomputation.
            # Whenever the Teacher is being synced (sync_ref_model=True branch),
            # voca_selection_mode == 2, and safe_tokens are not frozen, register one
            # of the two safe-token callbacks based on the steering mode.
            if self.args.voca_selection_mode == 2 and not self.args.freeze_safe_token:
                # The prompt-based init branch at the top of this method only loads
                # datasets when its own condition fires. To be safe regardless of which
                # branch ran above, reload here if any of the four splits is missing.
                if any(name not in locals() for name in
                       ("harmful_train", "harmless_train", "harmful_val", "harmless_val")):
                    harmful_train, harmless_train, harmful_val, harmless_val = \
                        self.load_and_sample_datasets_for_refusal_direction(args.seed)

                if self.use_refusal_vector:
                    safe_token_callback = DynamicSafeTokenViaRefusalVectorCallback(
                        trainer=self,
                        harmful_train=harmful_train,
                        harmless_train=harmless_train,
                        harmful_val=harmful_val,
                    ...
                    safe_token_callback = DynamicSafeTokenViaSystemPromptCallback(
                        trainer=self,
                        harmful_train=harmful_train,
                        harmless_train=harmless_train,
                        harmful_val=harmful_val,
                ...
                # Registered after DynamicRefusalVectorCallback so it sees the updated direction.
                self.add_callback(safe_token_callback)
                if self.accelerator.is_main_process:
                    mode_str = "refusal-vector" if self.use_refusal_vector else "system-prompt"
                    print(f"🔁 Registered dynamic safe-token callback ({mode_str} steering).")
    ...
    def _get_per_token_logps_and_entropies(
        ...
        # SafeSteer✅ 1. 获取当前模型所在的设备 (Student是cuda:0，Teacher是cuda:1)
