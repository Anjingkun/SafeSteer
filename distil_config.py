...
class DistilConfig(TrainingArguments):
    ...
    num_loss_tokens_to_keep: int = field(
        default=0,
        metadata={
            "help": "If > 0, only the first N tokens of each completion contribute to the loss (tokens at position "
            ">= N are masked out). Combined with `num_loss_tokens_to_skip=s`, the loss is applied to the interval "
            "`[s, N)`. A value of `0` (default) disables this cap and keeps all tokens."
        },
    )
    ...
    log_teacher_completions: bool = field(
        default=False,
        metadata={
            "help": "Debug switch: if True, also let the teacher generate completions and log them to wandb. "
            "Used to inspect the effect of refusal-vector / system-prompt injection on the teacher."
        },
    )
    ...
    model_name: str = field(
        default=None,
        metadata={
            "help": "This is the name or path of the model"
        }
    )
    use_refusal_vector: bool = field(
        default=False,
        metadata={
            "help": "whether to use refusal vector for training. If True, the refusal vector will be added to the model output before computing the loss."
        },)
    update_refusal_vector: bool = field(
        default=True,
        metadata={
            "help": "When use_refusal_vector=True: True (default) recomputes the refusal direction + safe_tokens every ref_model_sync_steps via DynamicRefusalVectorCallback; False freezes the init-time direction throughout training."
        },)
    freeze_safe_tokens: bool = field(
        default=True,
        metadata={
            "help": "When freeze_teacher=False: recompute safe tokens every ref_model_sync_steps."
        },)
    
    voca_selection_mode: int = field(
        default=None,
        metadata={
            "help": "This controls which tokens are selected for computing the loss. If 0 (default), all tokens are selected. If 1, only the top-100 token with the highest reward is selected. If 2, the top-100 safe tokens with the highest rewards are selected."
        }
    )

    voca_selection_num: int = field(
        default=None,
        metadata={
            "help": "The number of tokens to select for computing the loss when token_selection_mode is not 0."
        }
    )

    safe_token_horizon: int = field(
        default=1,
        metadata={
            "help": "Number of leading answer positions used when selecting safe tokens (voca_selection_mode=2). 1 = original behavior (only the first answer token); >1 teacher-forces baseline-greedy continuations and averages the delta log-prob across the first `safe_token_horizon` positions."
        }
    )
    ...
    selection_method: str = field(
        default="mean",
        metadata={
            "help": "Safe-token selection method when voca_selection_mode=2. 'mean' = average Δ over (sample,prompt,position) then top-k (legacy default). 'vote' = each (sample,prompt,position) casts top-K' votes; pick top-k by vote count (recommended for noisy / multi-position windows)."
        }
    )

    vote_top_k_inner: int = field(
        default=200,
        metadata={
            "help": "Per-(sample,prompt,position) top-K' used in vote-based safe_token selection. Only used when selection_method='vote'."
        }
    )
    ...
    num_samples_per_prompt: int = field(
        default=1,
        metadata={
            "help": "Replicate each harmless_val prompt N times and sample N independent baseline trajectories during safe_token selection. >1 enables do_sample inside _logits_over_horizon."
        }
    )

    safe_token_temperature: float = field(
        default=1.0,
        metadata={
            "help": "Sampling temperature for safe_token selection (only used when num_samples_per_prompt > 1). Distinct from the trainer's vLLM `temperature` (which controls completion generation)."
        }
    )

    safe_token_top_p: float = field(
        default=1.0,
        metadata={
            "help": "Top-p for safe_token sampling (only used when num_samples_per_prompt > 1). Distinct from the trainer's vLLM `top_p`."
        }
    )
    ...
    exclude_special_tokens: bool = field(
        default=True,
        metadata={
            "help": "If True, mask all special-token ids (tokenizer.all_special_ids ∪ vocab tokens whose decode contains '<|') before selecting safe_tokens. Prevents <|eot_id|>, <|im_end|>, <|start_header_id|> etc. from leaking into the safe_tokens list."
        }
    )

    min_steered_prob: float = field(
        default=1e-6,
        metadata={
            "help": "Minimum mean steered-Teacher probability a candidate token must reach (averaged over samples/prompts/positions) to be eligible as a safe_token. Tokens below this threshold are filtered out before top-k selection. 0 disables the filter."
        }
    )
    ...
    renormalize_safe_tokens: bool = field(
        default=False,
        metadata={
            "help": "Effective when voca_selection_mode!=0: if True, locally renormalize student/teacher log-probs over the selected-token slice (loss compares only the relative distribution inside selected set — empirically causes over-refusal). If False (default), keep absolute full-vocab log-probs so the KL preserves the absolute-mass signal."
        }
    )

    freeze_safe_token: bool = field(
        default=True,
        metadata={
            "help": "If True, the safe tokens are frozen and not updated during training."
        }
    )
